...
public class UserInfo {
	...
	// has_spouse = whether the user has a spouse recorded in the system
	// has_spouse_declaration = whether the user declared a spouse and is awaiting
	// acceptance from that person
	// has_spouse_declaration_to_confirm = whether another user has declared this
	// user as a spouse and is awaiting
	// confirmation from this user

	protected Boolean has_spouse, has_spouse_declaration, has_spouse_declaration_to_confirm;
	// Actual confirmed spouse or a declared one (by them or another)
	protected UserInfo spouse;

	// Spouse logic
	public Boolean unattached() {
		return (!has_spouse && !has_spouse_declaration && !has_spouse_declaration_to_confirm);
	}
	...
	public UserInfo getSpouse() {
		return spouse;
	}

	public Boolean hasSpouseDeclaration() {
		return has_spouse_declaration;
	}

	public Boolean hasSpouseDeclarationToConfirm() {
		return has_spouse_declaration_to_confirm;
	}

	public void declareSpouse(UserInfo proposed_spouse) {
		if (!has_spouse && !has_spouse_declaration && !has_spouse_declaration_to_confirm) {
			spouse = proposed_spouse;
			has_spouse_declaration = true;
		}
	}

	public void declareSpouseToConfirm(UserInfo claimed_spouse) {
		if (!has_spouse && !has_spouse_declaration && !has_spouse_declaration_to_confirm) {
			spouse = claimed_spouse;
			has_spouse_declaration_to_confirm = true;
		}
	}

	public void acceptSpouseDeclaration() {
		if (has_spouse_declaration_to_confirm) {
			has_spouse_declaration_to_confirm = false;
			has_spouse = true;
			spouse.spouseDeclarationAccepted();
		}
	}

	public void rejectSpouseDeclaration() {
		if (has_spouse_declaration_to_confirm) {
			has_spouse_declaration_to_confirm = false;
			spouse.spouseDeclarationRejected();
		}
	}

	protected void spouseDeclarationAccepted() {
		if (has_spouse_declaration) {
			has_spouse_declaration = false;
			has_spouse = true;
		}
	}

	protected void spouseDeclarationRejected() {
		if (has_spouse_declaration) {
			has_spouse_declaration = false;
		}
	}
