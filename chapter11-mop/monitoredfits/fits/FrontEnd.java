...
public class FrontEnd {
	...
	// * Declare a spouse
	public void USER_declareSpouse(Integer user_id, Integer spouse_id) {
		UserInfo user = backend.getUserInfo(user_id);
		UserInfo spouse = backend.getUserInfo(spouse_id);

		if (user.unattached() && spouse.unattached()) {
			user.declareSpouse(spouse);
			spouse.declareSpouseToConfirm(user);
		}
	}

	// * Accept a spouse declaration
	public void USER_acceptSpouseDeclaration(Integer user_id) {
		UserInfo user = backend.getUserInfo(user_id);

		if (user.hasSpouseDeclarationToConfirm()) {
			user.acceptSpouseDeclaration();
		}
	}

	// * Reject a spouse declaration
	public void USER_rejectSpouseDeclaration(Integer user_id) {
		UserInfo user = backend.getUserInfo(user_id);

		if (user.hasSpouseDeclarationToConfirm()) {
			user.rejectSpouseDeclaration();
		}
	}
