...
public class FrontEnd {
	...
	// * Transfer money to across users' accounts
	public Boolean ADMIN_transferBetweenAccounts(Integer uidSrc, String accnumSrc, Integer uidDst, String accnumDst,
			Double amount) {
		UserInfo from_u = backend.getUserInfo(uidSrc);

		if (from_u.getAccount(accnumSrc).getBalance() >= amount) {
			from_u.withdrawFrom(accnumSrc, amount);
	...
	// * Perform a compound transaction made up of a number of internal transactions
	public Boolean USER_compoundTransaction(Integer sid, CompoundTransaction ct) {
		return ct.execute(this, sid);
	}
