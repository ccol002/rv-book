...
public class Rule {
	...
	public void toAspectJOffline(AspectJScript aspectJrules) {
		aspectJrules.addBeforeAJRule("before (Joinpoint joinpoint): call(* Joinpoint.replay()) && target(joinpoint) {\n"
				+ "   if (joinpoint.matchFullName(\"" + getEvent() + "\") &&\n"
				+ "       joinpoint.getJoinpointKind().equals(JoinpointKind." + (before ? "BEFORE" : "AFTER") + ") &&\n"
				+ "       (" + getCondition() + ")) {\n" + "          " + getAction() + "\n" + "   }\n" + "}\n");

		// before *.replay() target (JoinPoint joinpoint)
		// | joinpoint.matchFullName("*.BackEnd.initialise") &&
		// joinpoint.getJoinpointKind().equals(JoinpointKind.BEFORE)
		// -> \{ Verification.fitsInitialisation(); \}
	}

	public void toAspectJOnline(AspectJScript aspectJrules) {
		toAspectJOnline(aspectJrules, null, null, null);
	}

	public void toAspectJOnline(AspectJScript aspectJrules, String foreach_var, ArrayList<String> vars,
			ArrayList<String> defaults) {
		String result = "";
