...
import rv.RVExceptionProperty1;
import rv.RVExceptionProperty2;
import rv.RVExceptionProperty3;
import rv.RVExceptionProperty6;
...
public class FrontEnd {
	...
	public void ADMIN_whitelistUser(Integer uid) {
		...
		try {
			u.makeWhitelisted();
		} catch (RVExceptionProperty6 rvX) {
	...
	public void ADMIN_makeGoldUser(Integer uid) {
		...
		} catch (RVExceptionProperty1 rvX) {
			// Suppress makeGoldUser to enforce
		}
	}
	...
	public Integer USER_login(Integer uid) {
		...
		if (u.isEnabled()) {
			...
			} catch (RVExceptionProperty2 rvX) {
				// Enforcement by injection of initialisation
				backend.initialise();
				return (u.openSession());
			}
		} else {
	...
	public Boolean USER_payToExternal(Integer uid, Integer sid, String accnumSrc, Double amount) {
		try {
			...
			return false;
		} catch (RVExceptionProperty3 rvX) {
			System.out.println("Reparation of Property 6 triggered");
			// Suppresses transfer and takes reparatory action
			backend.getUserInfo(uid).makeBlacklisted();
			backend.getUserInfo(uid).makeDisabled();
			return false;
		}
	...
	public Boolean USER_transferToOtherAccount(Integer uidSrc, Integer sidSrc, String accnumSrc, Integer uidDst,
		...
		try {
			...
			s.log("FAILED (not enough funds): " + "Payment of $" + amount + " from account <" + accnumSrc
					+ "> to account " + "<" + accnumDst + " of user " + uidDst);
			return false;
		} catch (RVExceptionProperty3 rvX) {
			System.out.println("Reparation of Property 3 triggered");
			// Suppresses transfer and takes reparatory action
			UserInfo u = backend.getUserInfo(uidSrc);
			if (!u.isBlacklisted())
				u.makeGreylisted();
			u.makeDisabled();
			return false;
