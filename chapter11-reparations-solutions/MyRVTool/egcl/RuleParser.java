	...
	static public ArrayList<Rule> parseRules(String s) throws Exception {
		...
		while (!s.equals("") && !s.startsWith("foreach")) {
			...
			if (s.startsWith("throws")) {
				...
				if (matcher.find()) {
					throws_exception = matcher.group(1);
					s = s.substring(throws_exception.length()).trim();
				} else {
					throw (new Exception("Error in exception name after keyword 'throws'"));
				}

				System.out.println("  - Throws [" + throws_exception + "]");

			} else {
				throws_exception = null;
				System.out.println("  - No exception named");
			}
			...
			rules.add(new Rule(before, event, parameter_variables, parameter_types, target_variable, target_type,
					throws_exception, condition, action));
		}
		return rules;
	}
